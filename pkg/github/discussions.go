...
func fragmentToDiscussion(fragment NodeFragment) *github.Discussion {
	return &github.Discussion{
		...
		// Sanitize title and category name to prevent XSS from untrusted user content (Defense in Depth for downstream LLM/MCP clients)
...
func GetDiscussion(t translations.TranslationHelperFunc) inventory.ServerTool {
	return NewTool(
		...
		func(ctx context.Context, deps ToolDependencies, _ *mcp.CallToolRequest, args map[string]any) (*mcp.CallToolResult, any, error) {
			...
			response := map[string]interface{}{
				"number": int(d.Number),
				// Sanitize title, body, and category name to prevent XSS from untrusted user content (Defense in Depth)
	...
	return NewTool(
		...
		func(ctx context.Context, deps ToolDependencies, _ *mcp.CallToolRequest, args map[string]any) (*mcp.CallToolResult, any, error) {
			...
			for _, c := range q.Repository.DiscussionCategories.Nodes {
				// Sanitize category name to prevent XSS from untrusted user content (Defense in Depth)
