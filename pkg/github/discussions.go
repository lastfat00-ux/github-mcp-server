...
func fragmentToDiscussion(fragment NodeFragment) *github.Discussion {
	return &github.Discussion{
		...
		// Sanitize title to prevent XSS from untrusted user content
		...
		func(ctx context.Context, deps ToolDependencies, _ *mcp.CallToolRequest, args map[string]any) (*mcp.CallToolResult, any, error) {
			...
			for _, c := range q.Repository.DiscussionCategories.Nodes {
				categories = append(categories, map[string]string{
					"id":   fmt.Sprint(c.ID),
					"name": sanitize.Sanitize(string(c.Name)), // Sanitize category name to prevent XSS from untrusted user content (defense-in-depth)
				})
			}
