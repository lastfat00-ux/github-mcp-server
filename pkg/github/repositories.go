...
func ListReleases(t translations.TranslationHelperFunc) inventory.ServerTool {
	return NewTool(
		...
		func(ctx context.Context, deps ToolDependencies, _ *mcp.CallToolRequest, args map[string]any) (*mcp.CallToolResult, any, error) {
			...
			// Sanitize release names and bodies to prevent XSS from untrusted user content
			for _, rel := range releases {
				if rel != nil {
					if rel.Name != nil {
						rel.Name = github.Ptr(sanitize.Sanitize(*rel.Name))
					}
					if rel.Body != nil {
						rel.Body = github.Ptr(sanitize.Sanitize(*rel.Body))
					}
				}
			}
...
func GetLatestRelease(t translations.TranslationHelperFunc) inventory.ServerTool {
	return NewTool(
		...
		func(ctx context.Context, deps ToolDependencies, _ *mcp.CallToolRequest, args map[string]any) (*mcp.CallToolResult, any, error) {
			...
			// Sanitize release name and body to prevent XSS from untrusted user content
			if release != nil {
				if release.Name != nil {
