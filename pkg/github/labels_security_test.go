...
func TestLabelsSanitization(t *testing.T) {
	t.Run("GetLabel sanitization", func(t *testing.T) {
		...
		maliciousLabel := map[string]any{
			"id":          githubv4.ID("label-id"),
			"name":        githubv4.String("Malicious <script>alert('xss')</script> Name"),
		...
		assert.NotContains(t, label["name"], "<script>")
		assert.NotContains(t, label["description"], "<script>")
		assert.Equal(t, "Malicious  Name", label["name"])
	...
	t.Run("LabelWrite success message sanitization", func(t *testing.T) {
		...
		mockedClient := githubv4mock.NewMockedHTTPClient(
			...
			githubv4mock.NewMutationMatcher(
				...
				githubv4.CreateLabelInput{
					...
					Name:         githubv4.String("<script>alert('xss')</script>"),
		...
		assert.Contains(t, textContent.Text, "label '' created successfully")
	})
}
