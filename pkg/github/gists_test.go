...
func Test_GistsXSS(t *testing.T) {
	...
	t.Run("ListGists sanitization", func(t *testing.T) {
		serverTool := ListGists(translations.NullTranslationHelper)
		mockGists := []*github.Gist{
			{
				ID:          github.Ptr("gist1"),
				Description: github.Ptr("Normal description with <b>bold</b> and <script>alert('xss')</script> malicious script"),
				HTMLURL:     github.Ptr("https://gist.github.com/user/gist1"),
			},
		}
		...
		// Check that <script> is stripped but <b> is preserved
		assert.NotContains(t, *returnedGists[0].Description, "<script>")
		assert.Contains(t, *returnedGists[0].Description, "<b>bold</b>")
	})

	// 2. Test GetGist sanitization
	t.Run("GetGist sanitization", func(t *testing.T) {
		...
		assert.Contains(t, *returnedGist.Description, "<b>bold</b>")
	})
}
