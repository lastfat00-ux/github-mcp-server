...
func ListNotifications(t translations.TranslationHelperFunc) inventory.ServerTool {
	return NewTool(
		...
		func(ctx context.Context, deps ToolDependencies, _ *mcp.CallToolRequest, args map[string]any) (*mcp.CallToolResult, any, error) {
			...
			// Sanitize notification titles using Defense in Depth to protect downstream LLM-based clients that render markdown/HTML directly
			for _, n := range notifications {
				if n != nil && n.Subject != nil && n.Subject.Title != nil {
					*n.Subject.Title = sanitize.Sanitize(*n.Subject.Title)
				}
			}
...
func GetNotificationDetails(t translations.TranslationHelperFunc) inventory.ServerTool {
	return NewTool(
		...
		func(ctx context.Context, deps ToolDependencies, _ *mcp.CallToolRequest, args map[string]any) (*mcp.CallToolResult, any, error) {
			...
			// Sanitize notification details title using Defense in Depth to protect downstream LLM-based clients that render markdown/HTML directly
			if thread != nil && thread.Subject != nil && thread.Subject.Title != nil {
				*thread.Subject.Title = sanitize.Sanitize(*thread.Subject.Title)
			}
