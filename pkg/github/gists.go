...
func ListGists(t translations.TranslationHelperFunc) inventory.ServerTool {
	return NewTool(
		...
		func(ctx context.Context, deps ToolDependencies, _ *mcp.CallToolRequest, args map[string]any) (*mcp.CallToolResult, any, error) {
			...
			// Sanitize descriptions against XSS (Defense in Depth) to protect downstream LLM-based clients
...
func GetGist(t translations.TranslationHelperFunc) inventory.ServerTool {
	return NewTool(
		...
		func(ctx context.Context, deps ToolDependencies, _ *mcp.CallToolRequest, args map[string]any) (*mcp.CallToolResult, any, error) {
			...
			// Sanitize description against XSS (Defense in Depth) to protect downstream LLM-based clients
			if gist != nil && gist.Description != nil {
				gist.Description = github.Ptr(sanitize.Sanitize(*gist.Description))
			}
