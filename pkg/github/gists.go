...
func ListGists(t translations.TranslationHelperFunc) inventory.ServerTool {
	return NewTool(
		...
		func(ctx context.Context, deps ToolDependencies, _ *mcp.CallToolRequest, args map[string]any) (*mcp.CallToolResult, any, error) {
			...
			// Defense in depth: Sanitize Gist descriptions to protect downstream clients from XSS
			for i := range gists {
				if gists[i] != nil && gists[i].Description != nil {
					gists[i].Description = github.Ptr(sanitize.Sanitize(*gists[i].Description))
				}
			}
...
func GetGist(t translations.TranslationHelperFunc) inventory.ServerTool {
	return NewTool(
		...
		func(ctx context.Context, deps ToolDependencies, _ *mcp.CallToolRequest, args map[string]any) (*mcp.CallToolResult, any, error) {
			...
			// Defense in depth: Sanitize Gist description to protect downstream clients from XSS
