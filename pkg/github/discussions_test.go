...
func Test_DiscussionSanitization(t *testing.T) {
	...
	t.Run("ListDiscussionCategories sanitization", func(t *testing.T) {
		...
		qCategories := "query($first:Int!$owner:String!$repo:String!){repository(owner: $owner, name: $repo){discussionCategories(first: $first){nodes{id,name},pageInfo{hasNextPage,hasPreviousPage,startCursor,endCursor},totalCount}}}"
		vars := map[string]interface{}{
			"owner": "owner",
			"repo":  "repo",
			"first": float64(25),
			...
			"repository": map[string]any{
				"discussionCategories": map[string]any{
					"nodes": []map[string]any{
						{
							"id":   "cat-1",
							"name": "Malicious <script>alert('xss')</script> Category",
						},
					},
					"pageInfo": map[string]any{
						"hasNextPage":     false,
		...
		matcher := githubv4mock.NewQueryMatcher(qCategories, vars, maliciousResponse)
		httpClient := githubv4mock.NewMockedHTTPClient(matcher)
		gqlClient := githubv4.NewClient(httpClient)
		deps := BaseDeps{GQLClient: gqlClient}
		handler := toolDef.Handler(deps)
