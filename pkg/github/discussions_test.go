...
func Test_DiscussionSanitization(t *testing.T) {
	t.Run("GetDiscussion sanitization", func(t *testing.T) {
		...
		cat, ok := out["category"].(map[string]interface{})
		require.True(t, ok)
		assert.Equal(t, "Category ", cat["name"])
	})
	...
	t.Run("ListDiscussionCategories sanitization", func(t *testing.T) {
			...
			"repository": map[string]any{
				"discussionCategories": map[string]any{
					"nodes": []map[string]any{
						{"id": "123", "name": "Category <script>alert('xss')</script> One"},
					},
					"pageInfo": map[string]any{
						"hasNextPage":     false,
						"hasPreviousPage": false,
		...
		assert.Equal(t, "Category  One", response.Categories[0]["name"])
	})
