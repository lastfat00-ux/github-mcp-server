...
func Test_DiscussionSanitization(t *testing.T) {
	t.Run("GetDiscussion sanitization", func(t *testing.T) {
		...
		require.True(t, ok)
		assert.Equal(t, "Malicious  Category", category["name"])
	})

	t.Run("ListDiscussions category sanitization", func(t *testing.T) {
		toolDef := ListDiscussions(translations.NullTranslationHelper)
		qListDiscussions := "query($after:String$first:Int!$owner:String!$repo:String!){repository(owner: $owner, name: $repo){discussions(first: $first, after: $after){nodes{number,title,createdAt,updatedAt,closed,isAnswered,answerChosenAt,author{login},category{name},url},pageInfo{hasNextPage,hasPreviousPage,startCursor,endCursor},totalCount}}}"
		vars := map[string]interface{}{
			"owner": "owner",
			"repo":  "repo",
			"first": float64(30),
		...
		matcher := githubv4mock.NewQueryMatcher(qListDiscussions, vars, maliciousResponse)
		httpClient := githubv4mock.NewMockedHTTPClient(matcher)
		gqlClient := githubv4.NewClient(httpClient)
		deps := BaseDeps{GQLClient: gqlClient}
		handler := toolDef.Handler(deps)
