...
func Test_SecretScanningAlertsXSS(t *testing.T) {
	// Setup mock alert with XSS payloads
	mockAlert := &github.SecretScanningAlert{
		Number:                             github.Ptr(42),
		State:                              github.Ptr("open"),
		HTMLURL:                            github.Ptr("https://github.com/owner/private-repo/security/secret-scanning/42"),
		ResolutionComment:                  github.Ptr("This is <script>alert('xss')</script><b>safe</b> comment"),
		PushProtectionBypassRequestComment: github.Ptr("I need <img src=x onerror=alert(1)><b>bypass</b> comment"),
		PushProtectionBypassRequestReviewerComment: github.Ptr("Approved with <b onmouseover=\"alert(1)\">reviewer</b> comment"),
	}

	t.Run("GetSecretScanningAlert sanitizes XSS", func(t *testing.T) {
		toolDef := GetSecretScanningAlert(translations.NullTranslationHelper)
		mockedClient := MockHTTPClientWithHandlers(map[string]http.HandlerFunc{
			GetReposSecretScanningAlertsByOwnerByRepoByAlertNumber: mockResponse(t, http.StatusOK, mockAlert),
		})
		...
		// Check that scripts and other unsafe tags are removed, but safe HTML remains
		assert.Equal(t, "This is <b>safe</b> comment", *returnedAlert.ResolutionComment)
		assert.Equal(t, `I need <img src="x"><b>bypass</b> comment`, *returnedAlert.PushProtectionBypassRequestComment)
		assert.Equal(t, "Approved with <b>reviewer</b> comment", *returnedAlert.PushProtectionBypassRequestReviewerComment)
	})

	t.Run("ListSecretScanningAlerts sanitizes XSS", func(t *testing.T) {
		...
		assert.Equal(t, "This is <b>safe</b> comment", *returnedAlerts[0].ResolutionComment)
		assert.Equal(t, `I need <img src="x"><b>bypass</b> comment`, *returnedAlerts[0].PushProtectionBypassRequestComment)
		assert.Equal(t, "Approved with <b>reviewer</b> comment", *returnedAlerts[0].PushProtectionBypassRequestReviewerComment)
	})
}
