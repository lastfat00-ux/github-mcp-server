...
func Test_ListNotifications(t *testing.T) {
	...
	mockMalicious := &github.Notification{
		ID: github.Ptr("456"),
	...
	tests := []struct {
		...
		},
		{
			...
			mockedClient: MockHTTPClientWithHandlers(map[string]http.HandlerFunc{
				GetNotifications: mockResponse(t, http.StatusOK, []*github.Notification{mockMalicious}),
			}),
			requestArgs:    map[string]interface{}{},
			expectError:    false,
			expectedResult: []*github.Notification{{ID: github.Ptr("456"), Subject: &github.NotificationSubject{Title: github.Ptr("Malicious  Title")}}},
		},
		{
			name: "error",
			mockedClient: MockHTTPClientWithHandlers(map[string]http.HandlerFunc{
...
func Test_GetNotificationDetails(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "success with sanitization",
			mockedClient: MockHTTPClientWithHandlers(map[string]http.HandlerFunc{
				GetNotificationsThreadsByThreadID: mockResponse(t, http.StatusOK, mockMalicious),
			}),
			requestArgs: map[string]interface{}{
				"notificationID": "456",
			},
			expectError:  false,
			expectResult: &github.Notification{ID: github.Ptr("456"), Subject: &github.NotificationSubject{Title: github.Ptr("Malicious  Title")}},
		},
		{
			name: "not found",
			mockedClient: MockHTTPClientWithHandlers(map[string]http.HandlerFunc{
